...
void napuniDomenskiObjekatIzGrafickogObjekta()   {
       ...
       r.setStanje(ef.getPanel().getStanje()); 
    
    }

// Promenljivo!!!
public void napuniGrafickiObjekatIzDomenskogObjekta(Rezervacija r){
    ...
    ef.getPanel().setDatumZavrsetka(r.getDatumZavrsetka());
       ef.getPanel().setStanje(r.getStanje());
...
Stanje odrediStanjeRezervacije()
{
    bbp.makeConnection();
    Stanje st = null;
    Rezervacija r1 = (Rezervacija)bbp.findRecord(r);
    bbp.closeConnection();
    if (r1 != null) 
        { if (r1.getStanje().equals("obradjen")) {st = new Obradjen(this);} 
          if (r1.getStanje().equals("neobradjen")) {st = new Neobradjen(this);}
          if (r1.getStanje().equals("storniran")) {st = new Storniran(this);} 
        }
        else
...
void obradiDomenskiObjekat(){
    Stanje st = odrediStanjeRezervacije();
...
public boolean nadjiDomenskiObjekat(){
 ...
 public boolean nadjiDomenskiObjekat1(){
    boolean signal;
    bbp.makeConnection();
    r = (Rezervacija)bbp.findRecord(r); 
    if (r != null) 
        { napuniGrafickiObjekatIzDomenskogObjekta(r);
          signal = true;
        }
