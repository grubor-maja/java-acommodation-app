...
import NonterminalExpression.SekSR;
import NonterminalExpression.SekSRPravilo1;
...
boolean preduslovNadjiDomenskiObjekat(){
    ApstraktniIzraz sbi = new SekSR("CCC");
    ApstraktniIzraz sbip1 = new SekSRPravilo1(sbi);
     if (!sbip1.Interpretiraj(String.valueOf(r.getSifraRezervacije()))) 
          { poruka  = "Neregularan izraz!!!";
              System.out.println("Neregularan izraz");
            prikaziPoruku();
            prikaziFormatRezervacije(sbi.getFormat());
            return false;
          }
     return true;
}

void prikaziFormatRezervacije(String format) 
{ 
  ef.getPanel().getSifraRezervacije1().setBackground(Color.red);   
  Timer timer = new Timer();
  
  timer.schedule(new TimerTask() {
  @Override
  public void run() {
    ef.getPanel().getSifraRezervacije1().setBackground(Color.white);
    ef.getPanel().setSifraRezervacije(format);
  }
}, 3000);
...
boolean nadjiDomenskiObjekat(){
    ...
    if (!preduslovNadjiDomenskiObjekat()) return false;    
    boolean signal;
    bbp.makeConnection();
